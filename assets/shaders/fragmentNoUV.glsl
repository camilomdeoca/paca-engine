...
void main()
{
    ...
    vec3 ambient = texture(u_diffuse, o_uvCoords).rgb * u_lightColor;

    vec3 normal = normalize(o_normal);
    vec3 lightDir = normalize(o_fragPosition - u_lightPosInViewSpace);
    float diff = dot(normal, -lightDir);
    vec3 diffuse = max(diff, 0.0) * u_lightColor * texture(u_diffuse, o_uvCoords).rgb;

    float specularStrength = 0.5;
    vec3 viewDir = normalize(o_fragPosition);
    vec3 reflectDirection = reflect(lightDir, normal);
    float spec = pow(max(dot(-viewDir, reflectDirection), 0.0), 32);
    vec3 specular = texture(u_specular, o_uvCoords).rgb * spec * u_lightColor;
